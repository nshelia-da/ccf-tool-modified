...
export default class ConsumptionManagementService {
  ...
  public accumulateKnownRowData(data: IMapper): void {
    const consumptionDetailRow = this.createConsumptionDetailRow(data)
    if (this.isUnsupportedUsage(consumptionDetailRow)) return
    if (this.isUnknownUsage(consumptionDetailRow)) return
    this.getEstimateByPricingUnit(consumptionDetailRow)
  }

  public getCostAndEstimateSingleRow(data: IMapper): IMapper {
    let estimates: { co2e: any; kilowattHours: any }
    const consumptionDetailRow = this.createConsumptionDetailRow(data)
    if (this.isUnsupportedUsage(consumptionDetailRow)) return data
