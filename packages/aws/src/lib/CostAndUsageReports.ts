...
export default class CostAndUsageReports {
  ...
  private createCostAndUsageReportFromDataMapper(
    data: IMapper,
  ): CostAndUsageReportsRow {
    return new CostAndUsageReportsRow(
      !!data.get('timestamp')
        ? new Date(data.get('timestamp') as string)
        : null,
      '',
      '',
      (data.get('accountId') as string) || '',
      (data.get('accountName') as string) || '',
      (data.get('region') as string) || '',
      (data.get('serviceName') as string) || '',
      (data.get('usageType') as never) != ''
        ? parseFloat(data.get('usageType') as string)
        : null,
      parseFloat(data.get('usageUnit') as string) || 0,
      parseFloat(data.get('vCpus') as string) || 0,
      {},
    )
  }

  accumulateKnownRowData(data: IMapper): IMapper {
    const costAndUsageReportRow =
      this.createCostAndUsageReportFromDataMapper(data)

    this.getFootprintEstimateFromUsageRow(costAndUsageReportRow, [])
    return data
  }

  getEstimatesForSingleRow(data: IMapper): IMapper {
    const costAndUsageReportRow =
      this.createCostAndUsageReportFromDataMapper(data)
    ...
    if (
      ...
      footprintEstimate = this.getEstimateForUnknownUsage(
        costAndUsageReportRow,
      ) as Record<any, any>
    } else {
      footprintEstimate = this.getFootprintEstimateFromUsageRow(
        costAndUsageReportRow,
        [],
      ) as Record<any, any>
    }
    ...
    if (footprintEstimate)
      Object.entries(footprintEstimate).forEach(([key, value]) =>
        data.set(key, value),
      )
      ...
      case KNOWN_USAGE_UNITS.ACU_HOUR:
        ...
        if (isNaN(computeFootprint.kilowattHours)) {
          ...
          return {
            ...
            kilowattHours: 0,
            co2e: isNaN(computeFootprint.co2e) ? 0 : computeFootprint.co2e,
